...
/* ====== WritableStream (WHATWG Streams, minimal-but-functional) ======
 * The sink counterpart to ReadableStream, enough for app bundles that build a
 * write target and push chunks through a writer:
 *   new WritableStream({ start(c), write(chunk, c), close(), abort(reason) })
 *   controller.error(e)
 *   stream.getWriter() -> writer.write(chunk)/close()/abort(reason)/releaseLock(),
 *                         writer.closed / writer.ready (both resolved promises)
 *   stream.locked / stream.abort(reason) / stream.close()
 *
 * Sink methods run synchronously (the same model ReadableStream's pull uses), so
 * write()/close()/abort() settle their promise immediately with undefined; a
 * sink error recorded via controller.error() (or a prior close) rejects later
 * writes, matching the spec's observable behaviour for the sync case. */

#define CLS_WRITABLESTREAM "WritableStream"

#define WS_SINK    "@@ws_sink"
#define WS_CLOSED  "@@ws_closed"
#define WS_ERR     "@@ws_err"
#define WS_WRITER  "@@ws_writer"
#define WS_STREAM  "@@ws_stream"
#define WS_CTL     "@@ws_ctl"

static bool ws_is_closed(var_t* stream) {
	var_t* c = get_obj(stream, WS_CLOSED);
	return (c != NULL && var_get_bool(c));
}

static bool ws_has_err(var_t* stream) {
	var_t* e = get_obj(stream, WS_ERR);
	return (e != NULL && e->type != V_UNDEF);
}

/* controller.error(e): record the error so later writes reject. */
static var_t* ws_ctl_error(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* ctl = get_obj(env, THIS);
	var_t* e = get_obj(env, "e");
	var_t* stream = get_obj(ctl, WS_STREAM);
	if(stream != NULL)
		rs_hidden(stream, WS_ERR, e ? e : var_new(vm));
	return NULL;
}

static var_t* ws_controller_of(vm_t* vm, var_t* stream) {
	var_t* ctl = get_obj(stream, WS_CTL);
	if(ctl != NULL) return ctl;
	ctl = var_new_obj(vm, NULL, NULL, NULL);
	rs_hidden(ctl, WS_STREAM, stream);
	vm_reg_native_on(vm, ctl, "error(e)", ws_ctl_error, NULL);
	var_add(ctl, "desiredSize", var_new_int(vm, 1));
	rs_hidden(stream, WS_CTL, ctl);
	return ctl;
}

/* Invoke a sink method synchronously. Args are supplied in natural order and
 * reversed for call_m_func (last arg at index 0), the convention used above. */
static void ws_call_sink(vm_t* vm, var_t* stream, const char* name, var_t* a0, var_t* a1) {
	var_t* sink = get_obj(stream, WS_SINK);
	if(sink == NULL) return;
	var_t* fn = get_obj(sink, name);
	if(fn == NULL || !fn->is_func) return;
	var_t* args = var_new_array(vm);
	if(a0 != NULL) var_array_add(args, a0);
	if(a1 != NULL) var_array_add(args, a1);
	var_array_reverse(args);
	vm->gc.gc_defer++;
	var_t* r = call_m_func(vm, sink, fn, args);
	vm->gc.gc_defer--;
	var_unref(args);
	if(r != NULL) var_unref(r);
}

static var_t* ws_writer_write(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* writer = get_obj(env, THIS);
	var_t* chunk = get_obj(env, "chunk");
	var_t* stream = get_obj(writer, WS_STREAM);
	if(stream == NULL)
		return promise_new_rejected(vm, var_new_str(vm, "TypeError: writer is not attached"));
	if(ws_has_err(stream))
		return promise_new_rejected(vm, get_obj(stream, WS_ERR));
	if(ws_is_closed(stream))
		return promise_new_rejected(vm, var_new_str(vm, "TypeError: stream is closed"));
	ws_call_sink(vm, stream, "write", chunk, ws_controller_of(vm, stream));
	if(ws_has_err(stream))
		return promise_new_rejected(vm, get_obj(stream, WS_ERR));
	return promise_new_resolved(vm, NULL);
}

static var_t* ws_writer_close(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* writer = get_obj(env, THIS);
	var_t* stream = get_obj(writer, WS_STREAM);
	if(stream == NULL)
	...
	if(ws_has_err(stream))
		return promise_new_rejected(vm, get_obj(stream, WS_ERR));
	if(ws_is_closed(stream))
		return promise_new_rejected(vm, var_new_str(vm, "TypeError: stream already closed"));
	rs_hidden(stream, WS_CLOSED, var_new_bool(vm, true));
	ws_call_sink(vm, stream, "close", NULL, NULL);
	return promise_new_resolved(vm, NULL);
}

static var_t* ws_writer_abort(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* writer = get_obj(env, THIS);
	var_t* reason = get_obj(env, "reason");
	var_t* stream = get_obj(writer, WS_STREAM);
	if(stream == NULL) return promise_new_resolved(vm, NULL);
	rs_hidden(stream, WS_ERR, reason ? reason : var_new(vm));
	ws_call_sink(vm, stream, "abort", reason, NULL);
	return promise_new_resolved(vm, NULL);
}

static var_t* ws_writer_releaseLock(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* writer = get_obj(env, THIS);
	var_t* stream = get_obj(writer, WS_STREAM);
	if(stream != NULL) {
		rs_hidden(stream, WS_WRITER, var_new(vm));
		var_add(stream, "locked", var_new_bool(vm, false));
	}
	rs_hidden(writer, WS_STREAM, var_new(vm));
	return NULL;
}

static var_t* ws_getWriter(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* stream = get_obj(env, THIS);
	var_t* lockedv = var_find_member_var(stream, "locked");
	if(lockedv != NULL && var_get_bool(lockedv)) {
		vm_throw_type_native(vm, "TypeError", "WritableStream is already locked");
		return var_new(vm);
	}
	var_t* writer = var_new_obj(vm, NULL, NULL, NULL);
	rs_hidden(writer, WS_STREAM, stream);
	vm_reg_native_on(vm, writer, "write(chunk)", ws_writer_write, NULL);
	vm_reg_native_on(vm, writer, "close()", ws_writer_close, NULL);
	vm_reg_native_on(vm, writer, "abort(reason)", ws_writer_abort, NULL);
	vm_reg_native_on(vm, writer, "releaseLock()", ws_writer_releaseLock, NULL);
	rs_hidden(writer, "closed", promise_new_resolved(vm, NULL));
	rs_hidden(writer, "ready", promise_new_resolved(vm, NULL));
	rs_hidden(stream, WS_WRITER, writer);
	var_add(stream, "locked", var_new_bool(vm, true));
	return writer;
}

static var_t* ws_abort(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* stream = get_obj(env, THIS);
	var_t* reason = get_obj(env, "reason");
	rs_hidden(stream, WS_ERR, reason ? reason : var_new(vm));
...
static var_t* ws_close(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* stream = get_obj(env, THIS);
	if(ws_has_err(stream))
		return promise_new_rejected(vm, get_obj(stream, WS_ERR));
...
static var_t* ws_constructor(vm_t* vm, var_t* env, void* data) {
	(void)data;
	var_t* stream = get_obj(env, THIS);
	var_t* sink = get_obj(env, "underlyingSink");
	rs_hidden(stream, WS_CLOSED, var_new_bool(vm, false));
	rs_hidden(stream, WS_ERR, var_new(vm));
	rs_hidden(stream, WS_WRITER, var_new(vm));
	if(sink != NULL && sink->type == V_OBJECT)
		rs_hidden(stream, WS_SINK, sink);
	var_add(stream, "locked", var_new_bool(vm, false));
	var_t* ctl = ws_controller_of(vm, stream);
	var_t* start = (sink != NULL) ? get_obj(sink, "start") : NULL;
	if(start != NULL && start->is_func) {
		var_t* a = var_new_array(vm);
		var_array_add(a, ctl);
		vm->gc.gc_defer++;
		var_t* r = call_m_func(vm, sink, start, a);
		vm->gc.gc_defer--;
		var_unref(a);
		if(r != NULL) var_unref(r);
	}
...
void reg_native_Stream(vm_t* vm) {
	...
	/* WritableStream */
	var_t* wcls = vm_new_class(vm, CLS_WRITABLESTREAM);
	vm_reg_native(vm, wcls, "constructor(underlyingSink, strategy)", ws_constructor, NULL);
	vm_reg_native(vm, wcls, "getWriter()", ws_getWriter, NULL);
	vm_reg_native(vm, wcls, "abort(reason)", ws_abort, NULL);
	vm_reg_native(vm, wcls, "close()", ws_close, NULL);
	vm_reg_var(vm, wcls, SYMKEY_TOSTRINGTAG, var_new_str(vm, "WritableStream"), true);
}
